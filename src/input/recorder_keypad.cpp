...
bool hasRecorderKeys(int fd)
{
    ...
    return testBit(key_bits, KEY_ESC) || testBit(key_bits, KEY_ENTER) || testBit(key_bits, KEY_KPENTER) ||
           ...
           testBit(key_bits, KEY_8) || testBit(key_bits, KEY_MUTE) || testBit(key_bits, KEY_VOLUMEDOWN) ||
           testBit(key_bits, KEY_VOLUMEUP);
}
...
RecorderKeypad::~RecorderKeypad()
{
    close();
}

bool RecorderKeypad::openDefault()
{
#if !LV_USE_SDL && defined(__linux__)
    if (const char* device_path = std::getenv("RECORDER_KEYBOARD_DEVICE")) {
    ...
    if (!opened) {
        spdlog::warn("RecorderKeypad: no keyboard input device opened");
    }
    return opened;
#else
    return false;
...
bool RecorderKeypad::openDevice(const std::string& path, bool require_recorder_keys)
{
#if !LV_USE_SDL && defined(__linux__)
    if (!ensureIndev()) {
        return false;
    ...
    if (require_recorder_keys && !hasRecorderKeys(fd)) {
        ::close(fd);
        return false;
    }

    const bool grab_input = envEnabled("RECORDER_KEYBOARD_GRAB", false);
    if (grab_input && ::ioctl(fd, EVIOCGRAB, 1) < 0) {
        spdlog::warn("RecorderKeypad: failed to grab {}: {}", path, std::strerror(errno));
        ::close(fd);
        return false;
    }

    _event_fds.push_back(fd);
    spdlog::info("RecorderKeypad: opened {}{}", path, grab_input ? " with grab" : "");
    return true;
#else
    (void)path;
    (void)require_recorder_keys;
    return false;
#endif
}

void RecorderKeypad::close()
{
#if !LV_USE_SDL && defined(__linux__)
...
void RecorderKeypad::poll()
{
    ...
    for (int fd : _event_fds) {
        while (true) {
            ...
            if (bytes_read < 0) {
                spdlog::warn("RecorderKeypad: failed to read input event: {}", std::strerror(errno));
            }
            break;
        }
    }
...
lv_indev_t* RecorderKeypad::indev() const
{
    return _indev;
}

void RecorderKeypad::setKeyCallback(KeyCallback callback)
{
...
bool RecorderKeypad::ensureIndev()
{
    if (_indev) {
        return true;
    }
    ...
    if (!_indev) {
        spdlog::error("RecorderKeypad: failed to create LVGL keypad indev");
        return false;
    }
