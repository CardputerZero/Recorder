...
public:
    ...
    RecorderKeypad(const RecorderKeypad&)            = delete;
    RecorderKeypad& operator=(const RecorderKeypad&) = delete;

    bool openDefault();
    bool openDevice(const std::string& path, bool require_recorder_keys);
    void close();
    void poll();
